...
public class SlideJigsawCaptchaTool : ICaptchaTool
{
    ...
    public async Task<CaptchaOutput> GetAsync(string captchaKey)
    {
        ...
        //底图
        using var baseImage = await Image.LoadAsync<Rgba32>(ToPath($@"{Directory.GetCurrentDirectory()}\wwwroot\captcha\jigsaw\backgrounds\{_random.Next(1, 6)}.jpg"));
        var randomTemplate = _random.Next(1, 7);
        //深色模板图
        using var darkTemplateImage = await Image.LoadAsync<Rgba32>(ToPath($@"{Directory.GetCurrentDirectory()}\wwwroot\captcha\jigsaw\templates\{randomTemplate}\dark.png"));
        //透明模板图
        using var transparentTemplateImage = await Image.LoadAsync<Rgba32>(ToPath($@"{Directory.GetCurrentDirectory()}\wwwroot\captcha\jigsaw\templates\{randomTemplate}\transparent.png"));
