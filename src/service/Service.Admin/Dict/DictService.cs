...
public class DictService : BaseService, IDictService, IDynamicApi
{
    ...
    public async Task<long> AddAsync(DictAddInput input)
    {
        ...
        if (input.Code != null && await _dictRepository.Select.AnyAsync(a => a.DictTypeId == input.DictTypeId && a.Code == input.Code))
        {
        ...
        if (input.Value != null && await _dictRepository.Select.AnyAsync(a => a.DictTypeId == input.DictTypeId && a.Value == input.Value))
        {
    ...
    public async Task UpdateAsync(DictUpdateInput input)
    {
        ...
        if (input.Code != null && await _dictRepository.Select.AnyAsync(a => a.Id != input.Id && a.DictTypeId == input.DictTypeId && a.Code == input.Code))
        {
            throw ResultOutput.Exception($"字典编码已存在");
        }

        if (input.Value != null && await _dictRepository.Select.AnyAsync(a => a.Id != input.Id && a.DictTypeId == input.DictTypeId && a.Value == input.Value))
        {
            throw ResultOutput.Exception($"字典值已存在");
        }
