...
<script lang="ts" setup name="admin/config/form">
import { reactive, toRefs, getCurrentInstance, ref } from 'vue'
import { ConfigAddInput, ConfigUpdateInput } from '/@/api/admin/data-contracts'
...
const state = reactive({
    showDialog: false,
    sureLoading: false,
    form: {} as ConfigAddInput & ConfigUpdateInput,
})
...
const open = async (row: any = {}) => {
    if (row.id > 0) {
        const res = await new ConfigApi().get({ id: row.id }, { loading: true }).catch(() => {
            proxy.$modal.closeLoading()
        })

        if (res?.success) {
            state.form = res.data as ConfigAddInput & ConfigUpdateInput
        }
    } else {
        state.form = { enabled: true } as ConfigAddInput & ConfigUpdateInput
    }
